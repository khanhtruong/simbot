...
fun MessageItem(
    ...
    var displayedIndex by rememberSaveable { mutableStateOf(0) }

    LaunchedEffect(message) {
        if (isLoaded) {
            displayedIndex = message.length
            return@LaunchedEffect
        }

        while (displayedIndex < message.length) {
            displayedIndex += 1
            delay(20)
        }
        onRenderComplete()
    ...
    Column(
        ...
        Surface(
            ...
            Text(
                text = message.substring(0, displayedIndex),
                fontSize = 16.sp,
                color = Color.White,
